...
muestra_promedios_ergodicos <- function(posterior, color_palette = "Blues", show_alpha = T) {
  if (show_alpha) {
...
mcmc_trace(
  ...
  n_warmup = 667, 
  facet_args = list(nrow = 2, labeller = label_parsed)
) + theme_minimal() + facet_text(size = 12) + labs(title = expression(beta))

mcmc_areas(
  fit_jerarquico, prob = 0.95
)
